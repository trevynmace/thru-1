// NORTHBOUND — Electron wrapper for desktop / Steam distribution.
// Boots the bundled Node server, then loads it in a pixel-friendly window.
...
function createWindow() {
  ...
  const tryLoad = (n = 0) => {
    win.loadURL(`http://localhost:${PORT}`).catch(() => {
      if (n < 24) setTimeout(() => tryLoad(n + 1), 250);
    });
  };
  tryLoad();
}
