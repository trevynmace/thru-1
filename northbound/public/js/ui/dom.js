 ...
 *
 * The `#id` may appear on any segment (`div#box.card` or `div.card#box`) — reading it
 * only off the tag silently turns `div.card#box` into a class literally named
 * "card#box" with no id, which fails in a way nothing warns you about.
 */
export function el(spec, props, ...children) {
  let tag = 'div';
  let id = '';
  const classes = [];
  for (const [i, seg] of String(spec).split('.').entries()) {
    const hash = seg.indexOf('#');
    const name = hash >= 0 ? seg.slice(0, hash) : seg;
    if (hash >= 0) id = seg.slice(hash + 1);
    if (i === 0) { if (name) tag = name; }
    else if (name) classes.push(name);
  }
